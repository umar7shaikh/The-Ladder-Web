  ...
  return (
    <section className="relative w-full bg-white py-20 lg:py-32 overflow-hidden">
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12">
        ...
        {/* Header Section */}
        <div className="flex flex-col items-center text-center gap-6 mb-24 md:mb-32">
          <div className="inline-flex items-center justify-center px-5 py-2 bg-[#FFE000] rounded-full shadow-sm">
            <span className="text-sm font-bold tracking-wide text-[#363A43]">OUR CORE VALUES</span>
          </div>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-[#363A43] font-poppins leading-tight">
        ...
        <div className="relative w-full max-w-6xl mx-auto">
          ...
          {/* The Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-y-12 md:gap-y-24 gap-x-12 lg:gap-x-64 relative z-10">
          ...
          <div className="hidden md:flex absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 items-center justify-center w-[600px] h-[600px] pointer-events-none z-0">
            ...
            <svg className="absolute w-full h-full" viewBox="0 0 600 600" fill="none" xmlns="http://www.w3.org/2000/svg">
              {/* COORDINATE LOGIC:
                  Center: 300, 300
                  We use L-shapes: Vertical out from center axis, curve, then Horizontal to cards.
              */}

              {/* Top Left L-Shape */}
              {/* Start (250, 280) -> Up to 150 -> Curve -> Left to 40 */}
            ...
            {/* Innermost circle reduced to w-100 to match smaller center circle */}
            <div className="absolute w-[340px] h-[340px] rounded-full bg-[#FFE000] opacity-5"></div>
            <div className="absolute w-[220px] h-[220px] rounded-full bg-[#FFE000] opacity-10"></div>
            <div className="absolute w-[100px] h-[100px] rounded-full bg-[#FFE000] opacity-20"></div>

            {/* Center Circle */}
            {/* Reduced from w-24 to w-16 */}
            <div className="relative w-16 h-16 bg-[#FFE000] rounded-full flex items-center justify-center shadow-sm z-20">
               {/* NEW LADDER SVG - Sized to w-8 (32px) */}
...
function ValueCard({ icon, title, description }: ValueCardProps) {
  return (
    <div className={`w-full bg-[#FFE000] rounded-[24px] p-8 md:p-10 flex flex-col gap-6 shadow-md hover:shadow-lg transition-shadow duration-300 relative z-10`}>
      <div className="w-14 h-14 bg-white rounded-full flex items-center justify-center text-[#363A43] flex-shrink-0">
        {icon}
      </div>
      <div className="space-y-3">
        <h3 className="text-xl md:text-2xl font-bold text-[#363A43] font-poppins">{title}</h3>
        <p className="text-[#363A43]/80 leading-relaxed font-medium text-sm md:text-base">
          {description}
        </p>
      </div>
    </div>
...
const BulbIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 21h6"></path><path d="M12 17v4"></path><path d="M12 1a6 6 0 0 0-6 6c0 3.866 6 12 6 12s6-8.134 6-12a6 6 0 0 0-6-6Z"></path></svg>
);
